<script type="text/javascript">
   var grid;
    ...
    var bulanStore = Ext.create('Ext.data.ArrayStore', {
        fields: ['index','bulan'],
        data : dataBulan 
        
    });
    
    var Taunstore = Ext.create('Ext.data.ArrayStore',{
        fields: ['index','tahun'],
        data : dataTaun
    }) 
   ...
   Ext.onReady(function(){
    ...
    Ext.define('DataGrid', {
                    extend: 'Ext.data.Model',
                    fields: ['booking_id','nama', 'no_ktp','time_start','discount','total','time_end','duration','status_bayar','keterangan','tanggal_booking'],
                    idProperty: 'threadid'
        });
        	DataStore = new Ext.data.Store({
              ...
              proxy: {
                    ...
                    type: 'ajax',
                    url: '<?php echo base_url()?>report/showreport_monthly',
                    reader: {
                        root: 'results',
                        totalProperty:'total',
                    },
        ...
        DataStore.load({params: 
		 {
			start: 0, 
			limit: row_per_page,
			dateMonthly: Ext.getCmp('bulan').getValue(),
			lapanganId : 0,
            tahun : 0
		 }
		});
